...
export function ServiceDetailPage({ serviceId, nav }: { serviceId: string; nav: Nav }) {
  ...
  const label = isCustom ? (customName || persistedCustomName || tr("自定义服务", "Custom service", "Dịch vụ tùy chỉnh")) : (svc ? localizeServiceLabel(serviceId, svc.label) : serviceId);
  ...
  const handleTest = async () => {
    ...
    if (isCustom && !baseUrl.trim()) {
      setStatus({ state: "error", message: tr("请先填写 Base URL", "Enter a base URL first", "Vui lòng điền Base URL trước") });
      return;
    }
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="space-y-5">
        ...
        {/* API Key — stored keys are never returned by the API; the field only
            shows whether one is set, so saving with an empty field keeps it. */}
        <Field label={apiKeyOptional ? tr("API Key（可选）", "API key (optional)", "API Key (tùy chọn)") : "API Key"}>
          <div className="relative">
            <input
              ...
              onChange={(e) => setApiKey(e.target.value)} placeholder={apiKeyOptional ? tr("本地服务可留空", "Optional for local service", "Có thể bỏ trống với dịch vụ cục bộ") : apiKeySet ? "••••••••" : "sk-..."}
          ...
          {apiKeySet && !apiKey && (
            <p className="text-xs text-muted-foreground mt-1">
              {tr("已保存密钥（为安全起见不回显）。留空保存则保留；删除请用「删除配置和密钥」。", "A key is saved (never shown again for security). Saving with an empty field keeps it; use the delete button to remove it.", "Khóa đã được lưu (không hiển thị lại vì lý do bảo mật). Lưu với ô trống sẽ giữ nguyên; muốn xóa hãy dùng “Xóa cấu hình và khóa”.")}
            </p>
          )}
        </Field>
            ...
            <span className="text-xs text-emerald-500">
              ...
              {detectedModel
                ? tr(
                  ...
                  `, tự khớp ${detectedModel}${detectedConfig ? ` / ${detectedConfig.apiFormat === "responses" ? "Responses" : "Chat"} / ${detectedConfig.stream ? "streaming" : "không streaming"}` : ""}`,
                )
                : ""}
            </span>
          )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label={tr("流式响应", "Streaming", "Streaming")}>
        ...
        {isConnected && (
          <div className="space-y-2">
            ...
            {models.length > 0 ? (
              ...
              <p className="text-xs text-muted-foreground/60">{tr("点击“测试连接”查看可用模型", "Click “Test connection” to list available models", "Bấm “Kiểm tra kết nối” để xem mô hình khả dụng")}</p>
            )}
          </div>
        )}
