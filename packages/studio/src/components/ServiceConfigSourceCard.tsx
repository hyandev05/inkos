...
export function ServiceConfigSourceCard({ onChange }: { onChange?: () => void }) {
  ...
  const importEnvConfig = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : tr("导入环境变量配置失败", "Failed to import env config", "Không nhập được cấu hình biến môi trường"));
    } finally {
      setImporting(false);
    }
  };
  ...
  const envLabel = envConfig.effectiveSource === "project"
    ? tr("项目 .env", "Project .env", "Dự án .env")
    : envConfig.effectiveSource === "global"
      ? tr("全局 ~/.inkos/.env", "Global ~/.inkos/.env", "Toàn cục ~/.inkos/.env")
      : null;
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground/70 mt-1">
            {tr("Studio 运行时：", "Studio runtime:", "Studio chạy bằng:")}
            <span className="text-foreground"> {tr("使用服务页配置和 Studio 密钥", "uses service page config and Studio keys", "dùng cấu hình trang service và khóa Studio")}</span>
          </div>
        </div>
      ...
      {storedConfigSource === "env" ? (
        <div className="rounded-lg border border-amber-500/25 bg-amber-500/[0.04] p-3 text-xs text-muted-foreground/80">
          {tr(
            ...
            "Cài đặt cũ đánh dấu `.env` được ưu tiên. Studio chạy sẽ bỏ qua; CLI, daemon và môi trường triển khai vẫn có thể dùng lớp override env.",
          )}
        </div>
      ) : null}
      ...
      {envDetected ? (
        <div className="rounded-lg border border-amber-500/25 bg-amber-500/[0.04] p-3 text-xs text-muted-foreground/80 space-y-1.5">
          <div className="text-foreground">
            {tr("检测到 LLM 环境变量覆盖：", "Detected LLM environment variable override:", "Phát hiện biến môi trường LLM override:")}
            <span className="font-medium"> {envLabel ?? tr("已检测到但未定位来源", "detected but source not located", "đã phát hiện nhưng chưa định vị nguồn")}</span>
          </div>
          ...
          <div className="text-muted-foreground/70 pt-1">
            {tr(
              ...
              "Dù đã phát hiện .env, Studio và các yêu cầu Agent không dùng trực tiếp lớp override này; bấm “Nhập cấu hình đã phát hiện” để lưu nó thành cấu hình service Studio.",
            )}
          </div>
        </div>
      ) : (
        <div className="rounded-lg border border-border/30 bg-secondary/20 p-3 text-xs text-muted-foreground/75">
          {tr(
            ...
            "Không phát hiện biến override LLM trong `.env` dự án hoặc toàn cục. Hiện dùng trực tiếp cấu hình dự án và cấu hình service Studio.",
          )}
        </div>
      )}
